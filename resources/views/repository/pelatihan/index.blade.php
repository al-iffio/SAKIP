...
<div class="btn-group d-flex border-bottom border-dark rounded-0" role="group" aria-label="Basic example">
  ...
  <button type="button" class="btn btn-dark rounded-0 flex-fill" style="background-color:#4b3d17" disabled>Pelatihan</button>
...
<div class="bg-white p-3 mb-3">
  <div class="border-bottom border-1 border-dark mb-3">
    ...
    <div class="btn-toolbar justify-content-between" role="toolbar" aria-label="Toolbar with button groups">
      ...
      <div class="input-group">
        <form action="/repository/pelatihan">
  ...
  <div class="table-responsive">
      <table class="table table-striped table-sm table-bordered">
        ...
        <tbody>
          @php 
            $no = 1 + (($pelatihans->currentPage()-1) * $pelatihans->perPage());
          @endphp
          @if ($pelatihans->count())
            @foreach ($pelatihans as $pelatihan)
            <tr>
              <td>{{ $no++ }}</td>
              <td>{{ $pelatihan->dokumenPelatihan }}</td>
              <td>
                <a href="{{ $pelatihan->link }}" target="_blank" class="badge bg-info"><span data-feather="eye"></span></a>
                <a href="/repository/pelatihan/{{ $pelatihan->id }}/edit" class="badge bg-warning"><span data-feather="edit"></span></a>
                <form action="/repository/pelatihan/{{ $pelatihan->id }}" method="post" class="d-inline">
                  @csrf
                  @method('delete')
        ...
        @else
          <table class="table table-striped table-sm table-bordered">
                <p class="text-center fs-6">Dokumen Pelatihan tidak ditemukan</p>
        @endif
          </table>
  </div>
</div>
...
<div class="d-flex justify-content-end">
  {!! $pelatihans->appends(Request::except('page'))->render() !!}
</div>

@include('repository.pelatihan.modalCreate')
