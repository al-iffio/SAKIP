...
<div class="btn-group d-flex border-bottom border-dark rounded-0" role="group" aria-label="Basic example">
  ...
  <button type="button" class="btn btn-dark rounded-0 flex-fill" style="background-color:#4b3d17" disabled>Template Dokumen</button>
</div>
...
<div class="bg-white p-3 mb-3">
  <div class="border-bottom border-1 border-dark mb-3">
    ...
    <div class="btn-toolbar justify-content-between" role="toolbar" aria-label="Toolbar with button groups">
      ...
      <div class="input-group">
        <form action="/repository/templateDokumen">
  ...
  <div class="table-responsive">
      <table class="table table-striped table-sm table-bordered">
        ...
        <tbody>
          @php 
            $no = 1 + (($templateDokumens->currentPage()-1) * $templateDokumens->perPage());
          @endphp
          @if ($templateDokumens->count())
            @foreach ($templateDokumens as $templateDokumen)
            <tr>
              <td>{{ $no++ }}</td>
              <td>{{ $templateDokumen->templateDokumen }}</td>
              <td>
                <a href="{{ $templateDokumen->link }}" target="_blank" class="badge bg-info"><span data-feather="eye"></span></a>
                <a href="/repository/templateDokumen/{{ $templateDokumen->id }}/edit" class="badge bg-warning"><span data-feather="edit"></span></a>
                <form action="/repository/templateDokumen/{{ $templateDokumen->id }}" method="post" class="d-inline">
                  @csrf
                  @method('delete')
        ...
        @else
          <table class="table table-striped table-sm table-bordered">
                <p class="text-center fs-6">Template Dokumen tidak ditemukan</p>
        @endif
          </table>
  </div>
</div>
...
<div class="d-flex justify-content-end">
  {!! $templateDokumens->appends(Request::except('page'))->render() !!}
</div>

@include('repository.templateDokumen.modalCreate')
